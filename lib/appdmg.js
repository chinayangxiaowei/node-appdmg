...
module.exports = exports = function (options) {
  ...
  pipeline.addStep('Copying keystone install', function (next) {
    if (!global.opts.keystoneInstall) return next.skip()

    const absolutePath = resolvePath(global.opts.keystoneInstall)

    function copyKeystoneInstall (next) {
      const finalPath = path.join(global.temporaryMountPath, path.basename(global.opts.keystoneInstall))
      fs.copyFile(absolutePath, finalPath, next)
    }

    util.pathExists(absolutePath, function (err, exists) {
      if (err) {
        return next(err)
      } else if (exists) {
        copyKeystoneInstall(next)
      }
    })
  })
